...
router.post('/:id', multipartMiddleware, async function(req, res){
  ...
  let info = {'filename':fileInfo.originalFilename,
    'path':fileInfo.path,
    'size':fileInfo.size,
    'type':fileInfo.type
  }
  ...
  let token = toks.getTokenStr();
    // 上传到服务器
  // var promise = addDocx2asset(req.params.id, fileInfo.path, token);
  ...
  let str = [{"template_name":fileInfo.originalFilename,
      "template_url:": 'http://127.0.0.1:80/' + fileInfo.originalFilename
    }];
